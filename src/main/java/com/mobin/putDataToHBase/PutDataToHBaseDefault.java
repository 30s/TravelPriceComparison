...
public class PutDataToHBaseDefault extends Mapper<LongWritable, Text, ImmutableBytesWritable, Put> {
    ...
    protected void map(LongWritable key, Text value,
        ...
        String rowKeyPrice = values[1]+"aAPRICEs"+values[2]+price; //price
        String rowKeyHotel = values[1]+"aAHOTELs"+values[2]+values[13]+new Random().nextInt(); //hotel
        Put put = new Put(Bytes.toBytes(rowKeyPrice));
      //  Put put1 = new Put(Bytes.toBytes(rowKeyHotel));
        put.addColumn(Bytes.toBytes("INFO"),Bytes.toBytes("URL"), Bytes.toBytes(values[0]));
      //  put1.addColumn(Bytes.toBytes("INFO"),Bytes.toBytes("URL"), Bytes.toBytes(values[0]));
